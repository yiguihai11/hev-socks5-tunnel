...
run_sniff_robustness_tests (void)
{
    ...
    /* Test 1: NULL queue */
    if (hev_filter_sniff_pcb_hostname (pcb, p_null, hostname,
                                       sizeof (hostname)) == -1) {
    ...
    if (hev_filter_sniff_pcb_hostname (pcb, p_empty, hostname,
                                       sizeof (hostname)) == -1) {
    ...
    if (p_bad) {
        ...
        if (hev_filter_sniff_pcb_hostname (pcb, p_bad, hostname,
                                           sizeof (hostname)) == -1) {
    ...
    struct pbuf *p_http =
        pbuf_alloc (PBUF_RAW, strlen (malformed_http), PBUF_RAM);
    if (p_http) {
        memcpy (p_http->payload, malformed_http, strlen (malformed_http));
        if (hev_filter_sniff_pcb_hostname (pcb, p_http, hostname,
                                           sizeof (hostname)) == -1) {
...
run_dns_qtype_extraction_tests (void)
{
    ...
    pos += 4;
    packet[pos++] = 3;
    memcpy (packet + pos, "com", 3);
    pos += 3;
    packet[pos++] = 0; // Null terminator
    packet[pos++] = 0;
    packet[pos++] = 1; // QTYPE A (1)
    packet[pos++] = 0;
    packet[pos++] = 1; // QCLASS IN (1)

    uint16_t qtype = extract_dns_qtype (packet, pos);
    TEST_ASSERT (qtype == 1);
    if (qtype == 1)
        printf ("    [ OK ] QTYPE A (1) extracted correctly\n");
    else
        printf ("    [ FAIL ] QTYPE A (1) extracted as %u\n", qtype);
    ...
    if (qtype == 28)
        printf ("    [ OK ] QTYPE AAAA (28) extracted correctly\n");
    else
        printf ("    [ FAIL ] QTYPE AAAA (28) extracted as %u\n", qtype);
}
