/*
 ============================================================================
 Name        : hev-test-enhanced-blacklist.c
 Author      : hev <r@hev.cc>
...
/* 模拟最小化环境用于测试 */
static ip_addr_t ip4_addr_from_str (const char *str) {
    ip_addr_t addr;
    ip4addr_aton (str, &addr);
    return addr;
}

static ip_addr_t ip6_addr_from_str (const char *str) {
    ip_addr_t addr;
    ip6addr_aton (str, &addr);
    return addr;
}

/* 简化的配置和日志函数 */
static int test_config_init(void) { return 0; }
static void test_config_fini(void) {}
static void test_log(int level, const char *fmt, ...) { (void)level; (void)fmt; }
static int test_config_get_blocked_ip_expiry(void) { return 60; }

#define LOG_D(fmt, ...) test_log(0, fmt, ##__VA_ARGS__)
#define LOG_I(fmt, ...) test_log(1, fmt, ##__VA_ARGS__)
#define LOG_E(fmt, ...) test_log(2, fmt, ##__VA_ARGS__)
#define hev_config_get_smart_proxy_blocked_ip_expiry_minutes test_config_get_blocked_ip_expiry
...
test_ip_blacklist (void)
{
    ...
    /* 测试IPv4 */
    test_ip = ip4_addr_from_str ("192.168.1.100");
    ...
    /* 测试IPv6 */
    test_ip = ip6_addr_from_str ("2001:db8::1");
...
test_removal (void)
{
    ...
    /* 添加一个条目然后删除 */
    test_ip = ip4_addr_from_str ("10.0.0.1");
...
test_compatibility (void)
{
    ...
    /* 测试旧接口 */
    test_ip = ip4_addr_from_str ("192.168.1.200");
