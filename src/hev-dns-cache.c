...
/* 简单哈希函数（DJB2） */
static uint32_t dns_hash(const char *str) {
    uint32_t hash = 5381;
    int c;
    while ((c = *str++)) {
        c = tolower(c);
...
/* 检测 IP 是否为国外 IP */
static int is_foreign_ip(const ip_addr_t *ip) {
    /* 使用 filter 模块的 is_domestic 函数 */
    return !hev_filter_is_domestic(ip);
}

/* 解析 DNS 响应，检测是否包含国外 IP（污染检测） */
static int detect_dns_pollution(const uint8_t *data, size_t len) {
    ...
    for (int i = 0; i < ancount && pos < len; i++) {
        ...
        if (rtype == DNS_TYPE_A && rdlen == 4) {
            ...
            if (is_foreign_ip(&ip)) {
                ...
                LOG_W("dns-cache: Detected foreign IP in DNS response: %s (pollution suspected)", ip_str);
                return 1; /* 污染 */
            }
        }
        /* 检查 AAAA 记录 */
            ...
            if (is_foreign_ip(&ip)) {
                ...
                ipaddr_ntoa_r(&ip, ip_str, sizeof(ip_str));
                LOG_W("dns-cache: Detected foreign IPv6 in DNS response: %s (pollution suspected)", ip_str);
                return 1; /* 污染 */
            }
        }
...
/* 提取 DNS 响应中的 TTL */
static uint32_t extract_dns_ttl(const uint8_t *data, size_t len) {
    if (len < sizeof(DNSHeader)) return 300; /* 默认 5 分钟 */
...
/* 提取 DNS 查询中的域名 */
static int extract_dns_domain(const uint8_t *data, size_t len, char *domain_out, size_t domain_max) {
...
void hev_dns_cache_fini(void) {
    ...
    for (int i = 0; i < DNS_CACHE_HASH_SIZE; i++) {
        ...
        while (entry) {
            ...
            if (entry->response_data) hev_free(entry->response_data);
            hev_free(entry);
            entry = next;
        }
    ...
    LOG_I("dns-cache: DNS cache module finalized (cleared %zu entries)", total_cache_entries);
}
...
int hev_dns_cache_insert(const char *domain, const uint8_t *response_data, 
    ...
    while (*current) {
        if (strcasecmp((*current)->domain, domain) == 0) {
            ...
            *current = new_entry;
            ...
            if (old->response_data) hev_free(old->response_data);
            if (old->is_poisoned) poisoned_cache_entries--;
            hev_free(old);
            
            if (is_poisoned) poisoned_cache_entries++;
            
            LOG_I("dns-cache: Updated cache for domain: %s (ttl=%u, poisoned=%d)", 
    ...
    LOG_I("dns-cache: Inserted cache for domain: %s (ttl=%u, poisoned=%d, total=%zu)", 
...
/* DNS 响应监控任务 */
static void dns_response_monitor_task(void *data) {
    HevDNSPoisonContext *ctx = (HevDNSPoisonContext *)data;
    uint8_t buffer[2048];
    struct sockaddr_storage remote_addr;
    socklen_t addr_len;
    
    LOG_D("dns-cache: DNS response monitor started for domain: %s", ctx->domain);
    ...
    if (recv_len > 0) {
        ...
        if (is_poisoned) {
            ...
            LOG_I("dns-cache: DNS response is clean for domain: %s, caching...", ctx->domain);
        ...
        LOG_W("dns-cache: No DNS response received for domain: %s (timeout)", ctx->domain);
    }
...
cleanup:
    /* 清理上下文 */
    if (ctx->original_query) pbuf_free(ctx->original_query);
    hev_free(ctx);
...
int hev_dns_poison_detect_and_handle(struct udp_pcb *pcb, struct pbuf *p,
    ...
    if (hev_dns_cache_lookup(domain, &cached_response, &cached_len)) {
        /* 缓存命中，直接响应 */
        struct pbuf *response = pbuf_alloc(PBUF_TRANSPORT, cached_len, PBUF_RAM);
        if (response) {
