...
/* 唤醒所有 WAITING 状态的任务，用于关闭时清理 */
static inline void
hev_task_system_wakeup_all_waiting_tasks (HevTaskSystemContext *ctx)
{
    HevListNode *node;

    /* 遍历所有任务，唤醒处于 WAITING 状态的任务 */
    node = hev_list_first (&ctx->all_tasks);
    while (node) {
        HevTask *task = container_of (node, HevTask, list_node);

        /* 只唤醒 WAITING 状态的任务，跳过 RUNNING 和 STOPPED */
        if (task->state == HEV_TASK_WAITING) {
            /* 唤醒这个任务 */
...
hev_task_system_pick_current_task (HevTaskSystemContext *ctx)
{
    ...
    if (ctx->running_task_count < ctx->total_task_count) {
        if (ctx->running_task_count) {
            ...
            /* 当没有运行任务但总任务数 > 0 时，说明有任务在等待 I/O。
             * 在关闭场景下，这些任务可能需要被唤醒以检查退出条件。 */
            int timeout = 100;
            int iterations = 0;

            do {
                hev_task_system_io_poll (ctx, timeout);

                /* If all tasks are gone, exit the loop */
                if (ctx->total_task_count == 0)
                    break;

                /* 如果经过几轮等待后仍然没有运行任务，唤醒所有等待的任务
                 * 这样它们可以检查退出条件并正常退出 */
                if (iterations == 3) { /* 300ms */
                    hev_task_system_wakeup_all_waiting_tasks (ctx);
                }

                /* 如果唤醒后仍然没有运行任务，可能是任务正在退出中，
                 * 给它们更多时间完成清理 */
                if (iterations > 30) { /* 3 seconds */
                    /* Force exit by setting current_task to NULL */
